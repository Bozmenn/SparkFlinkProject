...
object MainProcess {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //kafkaProps.setProperty("bootstrap.servers", System.getenv("KAFKA_BOOTSTRAP_SERVERS"))
    //kafkaProps.setProperty("group.id", System.getenv("KAFKA_GROUP_ID"))
  ...
  def calcStreamAgg(orders: DataStream[Order]): DataStream[(Long, Long, String, Timestamp)] = {
    orders
      .map(order => (order.seller_id, order.product_id, order.location, order.order_date))
      .keyBy(_._3)
